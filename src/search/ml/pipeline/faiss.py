...
class FaissPipeline:
    ...
    def __del__(self):
        ...
        for key in self.dataframes:
            ...
            self.save(database=cached_data[0], index_id=cached_data[1],
                      df=df)
    ...
    def __getDataframeFromCache(self, database, index_id):
        ...
        if self.dataframes.get(cache_key) is not None \
    ...
    def __getDatasetFromDisk(self, database, index_id):
        target_path = '{}/{}/{}'.format(self.storage, database,
                index_id)
        if os.path.exists(target_path):
            self.logger.info('__getDatasetFromDisk: {} for database: {} index_id: {}'.format(target_path,
                             database, index_id))
    ...
    def __setDatasetToCache(
        ...
        cache_key = 'cache_faiss_{}_{}'.format(database, index_id)
        self.logger.info('Caching embedding count: {} for database: {} index_id: {}'.format(ds.num_rows,
                         database, index_id))
    ...
    def __setDataframeToCache(
        ...
        cache_key = 'cache_dataframe_{}_{}'.format(database, index_id)
        self.logger.info('Caching dataframe cache_key: {} for database: {} index_id: {}'.format(cache_key,
                         database, index_id))
    ...
    def __saveDatasetToDisk(
        ...
        target_path = '{}/{}/{}'.format(self.storage, database,
                index_id)
        self.logger.info('Saving with embedding count: {} for database: {} index_id: {} to path: {}'.format(ds.num_rows,
                         database, index_id, target_path))
    ...
    def __dropDataframeRow(
        ...
        if df.empty is False:
            if is_string_dtype(df[column_name]):
                df.drop(df.index[df[column_name] == str(column_value)],
                        inplace=True)
            if is_numeric_dtype(df[column_name]):
                df.drop(df.index[df[column_name] == int(column_value)],
                        inplace=True)
    ...
    def __validate(self, items):
        ...
        for item in items:
            ...
            if key is None:
                ...
                if key != index_id:
                    error_message = \
                        'Items in dataset has more than one index:{}'.format(json.dumps(items))
    ...
    def get(self, database, index_id):
        ...
        if self.storage_location == StorageLocation.MEMORY_CACHE:
            ds = self.__getDatasetFromCache(database=database,
                    index_id=index_id)
        if self.storage_location == StorageLocation.LOCAL_DISK:
            ds = self.__getDatasetFromDisk(database=database,
                    index_id=index_id)
        if ds is not None:
            return ds
        else:
    ...
    def write(
        ...
        df = self.__getDataframeFromCache(database=database,
                index_id=index_id)
        self.logger.info('ml.pipeline.faiss-get-dataframe-from-cache-in-{}-seconds-during-{}'.format(time.time()
                         - start_time, datetime.datetime.now()))
        if df is None:
            ds = self.get(database=database, index_id=index_id)
            df = pd.DataFrame(ds)
            self.logger.info('ml.pipeline.faiss-get-dataframe-from-dataset-on-disk-in-{}-seconds-during-{}'.format(time.time()
                             - start_time, datetime.datetime.now()))
        for record in records:
            if record.get('embedding') is not None:
                record['embedding'] = \
                    numpy.array(json.loads(record['embedding']))
            if record.get('context_embedding') is not None:
                record['context_embedding'] = \
                    numpy.array(json.loads(record['context_embedding']))
            if record.get('label_embedding') is not None:
                record['label_embedding'] = \
                    numpy.array(json.loads(record['label_embedding']))
            df_item = pd.DataFrame([record])
            if df_item is not None and record['operation'] == 'DELETE':
                self.__dropDataframeRow(df=df,
                ...
                df = df.drop_duplicates(subset=['database', 'index_id',
                        '_id'], keep='last')
                df = df.drop_duplicates(subset=['database', 'index_id',
                        'operation', 'primary_key_name',
                        'primary_key_value'], keep='last')
        self.__setDataframeToCache(database=database,
                                   index_id=index_id, df=df)
        self.logger.info('ml.pipeline.faiss-updated-dataset-in-{}-seconds-during-{}'.format(time.time()
                         - start_time, datetime.datetime.now()))
        self.save(database=database, index_id=index_id, df=df)
        self.logger.info('ml.pipeline.faiss-completed-in-{}-seconds-during-{}'.format(time.time()
                         - start_time, datetime.datetime.now()))
        return df
    ...
    def save(
        ...
        if df is not None:
            ...
            if ds is not None and self.storage_location \
                == StorageLocation.MEMORY_CACHE:
                self.__setDatasetToCache(database=database,
                        index_id=index_id, ds=ds)
            if ds is not None and self.storage_location \
                == StorageLocation.LOCAL_DISK:
                self.__saveDatasetToDisk(database=database,
                        index_id=index_id, ds=ds)
    ...
    def __getSamplesForText(
        ...
        (scores, samples) = ds.get_nearest_examples('embedding',
                embedding, k=limit)
        samples_df = pd.DataFrame.from_dict(samples)
        samples_df['scores'] = scores
        samples_df.sort_values('scores', ascending=False, inplace=True)
        for (_, row) in samples_df.iterrows():
            prediction = {'id': row.primary_key_value,
    ...
    def __getSamplesForLabel(
        ...
        predictions = []
        ds.add_faiss_index(column='label_embedding',
                           metric_type=faiss.METRIC_INNER_PRODUCT)
        (scores, samples) = ds.get_nearest_examples('label_embedding',
                label_embedding, k=limit)
        samples_df = pd.DataFrame.from_dict(samples)
        samples_df['scores'] = scores
        samples_df.sort_values('scores', ascending=False, inplace=True)
        for (_, row) in samples_df.iterrows():
    ...
    def __getSamplesForImages(
        ...
        predictions = []
        ds.add_faiss_index(column='context_embedding',
                           metric_type=faiss.METRIC_L2)
        (scores, samples) = ds.get_nearest_examples('context_embedding'
                , context_embedding, k=limit)
        samples_df = pd.DataFrame.from_dict(samples)
        ...
        for (_, row) in samples_df.iterrows():
            prediction = {'id': row.primary_key_value,
                          'text': row.column_value, 'score': row.scores}
            predictions.append(prediction)
        return predictions
    ...
    def search(
        ...
        if ds is None or ds.num_rows == 0:
            return predictions
        if primary_key_values is not None and primary_key_values \
            != 'None':
            try:
        ...
        (text_embedding, context_embedding, label_embedding,
         text_generated, labels_generated) = \
            self.embed_pipeline.encode(context)
        has_context_embedding = \
            self.embed_pipeline.hasContextEmbedding(context)
        if has_context_embedding is True:
            return self.__getSamplesForImages(ds=ds,
                    context_embedding=context_embedding, limit=limit)
        if text_embedding is not None:
            text_predictions = self.__getSamplesForText(ds=ds,
                    embedding=text_embedding, limit=limit)
            predictions = predictions + text_predictions
        if label_embedding is not None:
            label_predictions = self.__getSamplesForLabel(ds=ds,
                    label_embedding=label_embedding, limit=limit)
