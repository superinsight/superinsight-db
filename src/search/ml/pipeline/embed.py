...
class EmbedPipeline:
  ...
  def encode(self, text):
    if validators.url(text):
      ...
      return text_embedding, context_embedding, label_embedding, text_generated, labels_generated
    else:
    ...
    try:
      with urllib.request.urlopen(text) as response:
        ...
        if info.get_content_maintype() == "image":
          ...
          image_path = image_dir + '/0/0.' + info.get_content_subtype().lower()
      ...
      class ImageFolderWithPaths(datasets.ImageFolder):
          def __getitem__(self, index):
              return super(ImageFolderWithPaths,
                           self).__getitem__(index) + (self.imgs[index][0], )
