...
class ImageToTextPipeline:
	...
	def generate(self, text):
		if validators.url(text):
			...
			labels = []
			if image_tags is not None and len(image_tags) > 0 and isinstance(image_tags, list):
	...
	def __imageToText(self, image_url):
		if Environment.index_image_to_caption is False:
			return None
		payload = json.dumps({ "inputs": image_url, "model": Environment.image_to_text_model, "options": { "use_gpu": False,"wait_for_model": True } })
	...
	def __imageToTags(self, image_url):
		if Environment.index_image_to_label is False:
			return None
		payload = json.dumps({ "inputs": image_url, "model": Environment.image_classification_model, "options": { "use_gpu": False,"wait_for_model": True } })
		headers = { "accept": "application/json", "Content-Type": "application/json"}
