...
class DatabaseConsumer:
  ...
  def divide(self, message):
    ...
    for item in items:
      future_embed = self.producer_conquer.send(Environment.kafka_topic_conquer, key = item["index_id"], value={
          "_id": item["_id"],
          "source": batch_id,
    ...
    self.logger.info(
        "database.consumer.divide-completed-in-{}-seconds".format(time.time() - start_time))

  def conquer(self, message):
    start_time = time.time()
    item = json.loads(message)
    text_embedding, context_embedding, label_embedding, text_generated, labels_generated = EmbedPipeline().encode(text = item["text"])
    ...
    self.logger.info(
        "database.consumer.conquer-completed-in-{}-seconds".format(time.time() - start_time))
  ...
  def combine(self, message):
    ...
    if self.batchHasCompleted(items) is True:
      ...
      self.faiss_pipeline.write(
          database=database, index_id=index_id, items=items)
      if os.path.exists(target_path):
          os.remove(target_path)
      self.logger.info("database.consumer.combine-has-batched-completed-{}-seconds-during-{}".format(
          time.time() - start_time, datetime.datetime.now()))

    self.logger.info("database.consumer.combine-completed-in-{}-seconds-during-{}".format(
        time.time() - start_time, datetime.datetime.now()))
