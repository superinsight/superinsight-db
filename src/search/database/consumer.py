...
class DatabaseConsumer:
    ...
    def consume(self, topics=None,
                storage_location=StorageLocation.LOCAL_DISK):
        self.consumer = \
            KafkaConsumer(bootstrap_servers=Environment.kafka_bootstrap_servers,
                          group_id=Environment.kafka_group_default,
                          auto_offset_reset='earliest')
        self.producer_conquer = \
            KafkaProducer(bootstrap_servers=Environment.kafka_bootstrap_servers,
                          key_serializer=str.encode,
                          value_serializer=lambda v: \
                          json.dumps(v).encode('utf-8'))
        self.producer_combine = \
            KafkaProducer(bootstrap_servers=Environment.kafka_bootstrap_servers,
                          key_serializer=str.encode,
                          value_serializer=lambda v: \
                          json.dumps(v).encode('utf-8'))
        self.logger.info('Consuming topic: {} for group id: {}'.format(topics,
                         Environment.kafka_group_default))
        self.storage_location = storage_location
        self.faiss_pipeline = \
            FaissPipeline(storage_location=self.storage_location)
        try:
    ...
    def divide(self, message):
        ...
        if self.storage_location == StorageLocation.LOCAL_DISK:
            directory = \
                '{}/.processing'.format(Environment.default_storage)
        ...
        for item in items:
            future_embed = \
                self.producer_conquer.send(Environment.kafka_topic_conquer,
                    key=item['index_id'], value={
                '_id': item['_id'],
                'source': batch_id,
        ...
        self.logger.info('database.consumer.divide-completed-in-{}-seconds'.format(time.time()
                         - start_time))
    ...
    def conquer(self, message):
        ...
        item['context_embedding'] = \
            json.dumps(context_embedding.tolist())
        ...
        key = item['index_id']
        future_emerge = \
            self.producer_combine.send(Environment.kafka_topic_combine,
                key=key, value=item)
        future_emerge.get(timeout=60)
        self.producer_combine.flush()
        self.logger.info('database.consumer.conquer-completed-in-{}-seconds'.format(time.time()
                         - start_time))
    ...
    def combine(self, message):
        ...
        if self.storage_location == StorageLocation.LOCAL_DISK:
            target_path = \
                '{}/.processing/{}.json'.format(Environment.default_storage,
                    batch_id)
            with open(target_path, 'w') as outfile:
                outfile.write(json.dumps(items))
        if self.batchHasCompleted(items) is True:
            ...
            self.faiss_pipeline.write(database=database,
                    index_id=index_id, items=items)
            if os.path.exists(target_path):
                os.remove(target_path)
            self.logger.info('database.consumer.combine-has-batched-completed-{}-seconds-during-{}'.format(time.time()
                             - start_time, datetime.datetime.now()))

        self.logger.info('database.consumer.combine-completed-in-{}-seconds-during-{}'.format(time.time()
                         - start_time, datetime.datetime.now()))

    def batchHasCompleted(self, items):
        completed = [item for item in items if item.get('embedding')
                     is not None]
