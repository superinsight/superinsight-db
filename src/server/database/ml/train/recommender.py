...
class TrainRecommender:
  ...
  def __init__(self, logger=None, handler=None, database=None):
    ...
    self.logger.info("Init ML.Train.TrainRecommender...")
    self.database = database
    self.producer = KafkaProducer(bootstrap_servers=Environment.kafka_bootstrap_servers, key_serializer=str.encode,
  ...
  def formatForStreaming(self, model_id, rows):
    ...
    for row in rows:
      batch.append({
          ...
          "model_id": model_id,
          "total": total,
          "category_id": row["category_id"] if row.get("category_id") is not None else None,
          "user_id": row["user_id"],
          "item_id": row["item_id"],
          "timestamp": row["timestamp"] if row.get("timestamp") is not None else None
      })
