...
class Recommender:
    ...
    def predict(self, inputs, size = 10, model_id = None, database = None, database_user = None):
      ...
      for input in inputs:
        ...
        for recommended_item in user_predictions:
          predictions.append((input, recommended_item["item_id"], recommended_item["score"]))
      sqlHelper = SqlHelper(database=database)
      sql, table_schema, table_name = SqlGenerator().scriptCreateRecommenderPredictOutputTable(predict_id=predict_id, database_user = database_user)
      ...
      for predicted in predictions:
        ...
        sql_insert = SqlGenerator().scriptInsertRecommenderPredictOutputTable(predict_id=predict_id, user_id=user_id, item_id=item_id, score=score)
    ...
    def http_request(self, database, model_id, input, size):
      ...
      if self.cache.get(cache_key) is None:
        url = "{}/{}/{}/user/{}?size={}".format(Environment.provider_api_url_superinsight_recommender, database, model_id, input, size)
