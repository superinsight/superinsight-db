...
class Summarization:
    ...
    def predict(self, inputs, min_length = 0, database = None, database_user = None):
      ...
      sql, table_schema, table_name = SqlGenerator().scriptCreateSummarizationPredictOutputTable(predict_id=predict_id, database_user = database_user)
      sqlHelper.execute(sql)
      script_for_inserts = ""
      for predicted in predictions:
        input = predicted[0]
        summary = predicted[1]
        sql_insert = SqlGenerator().scriptInsertSummarizationPredictOutputTable(predict_id=predict_id, input=input, summary=summary)
