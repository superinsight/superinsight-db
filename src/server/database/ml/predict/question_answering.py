...
class QuestionAnswering:
    ...
    def predict(self, inputs,  question, database=None, database_user = None):
      ...
      sql, table_schema, table_name = SqlGenerator().scriptCreateQuestionAnsweringPredictOutputTable(predict_id=predict_id, database_user = database_user)
      self.logger.debug(sql)
      sqlHelper.execute(sql)
      script_for_inserts = ""
      for predicted in predictions:
        if predicted[0] is not None and predicted[1] is not None and predicted[2] is not None:
          score = predicted[2]["score"]
          start = predicted[2]["start"]
          end = predicted[2]["end"]
          answer = predicted[2]["answer"]
          sql_insert = SqlGenerator().scriptInsertQuestionAnsweringPredictOutputTable(predict_id=predict_id, input = predicted[0], question = predicted[1], answer = answer, score = score, start = start, end = end)
