...
class TableLogger:
  ...
  def __init__(self, logger = None, handler = None, database = None):
    ...
    if database is None:
      raise Exception("database has not been assigned for SQL.TableLogger")
    else:
      self.database = database
      self.logger.info("Init SQL.TableLogger for database: {}...".format(database))
    self.database = database
  ...
  def readRecentUpdates(self, primary_key_name, primary_key_value, version_after_id):
    if version_after_id is None:
      version_after_id = 0
    recent_updates = self._readRecentUpdates(primary_key_name = primary_key_name, primary_key_value = primary_key_value, version_after_id = version_after_id)
    version = 0
    tables = []
    if recent_updates is not None and len(recent_updates) > 0:
      ...
      for changed in recent_updates:
        id = "{}_{}_{}".format(changed["table_schema"], changed["table_name"], changed["column_name"])
  ...
  def _readRecentUpdates(self, primary_key_name, primary_key_value, version_after_id):
    if version_after_id is None:
      version_after_id = 0
    self.logger.info("TableLogger.readRecentUpdates Primary Key: {}={} and version_after_id {}".format(primary_key_name, primary_key_value, version_after_id))
    changes = []
    self.logger.info(self.postgres_user,
                                  self.postgres_password,
                                  self.postgres_host,
                                  self.postgres_port,
                                  self.postgres_database)
    conn = psycopg2.connect(user=self.postgres_user,
    ...
    try:
      ...
      ORDER BY "_id" ASC 
      LIMIT {}
      """.format(self.postgres_system_schema, version_after_id, self.postgres_system_schema, primary_key_name, primary_key_value, self.table_logs_read_size)
      ...
      self.logger.error("Error while fetching data from PostgreSQL", error)
        ...
        if conn:
            cursor.close()
            conn.close()
            self.logger.error("PostgreSQL conn is closed")
        return changes 
  ...
  def readTables(self):
    ...
    try:
      ...
      """.format(self.postgres_system_schema, self.postgres_system_schema)
      cursor.execute(postgreSQL_select_Query)
      records = cursor.fetchall()
      for row in records:
        table = {}
      ...
      self.logger.error("TableLogger.readTables:except - Error while fetching data from PostgreSQL", error)
      ...
      if conn:
        cursor.close()
        conn.close()
        self.logger.info("TableLogger.readTables:finally - PostgreSQL conn is closed")
      return tables

      
  def insertTableLog(self, last_row_id, operation, primary_key_name, primary_key_value):
    ...
    try:
      ...
      cur.execute(sql.format(self.postgres_system_schema, self.postgres_system_schema, primary_key_name, primary_key_value, last_row_id, operation))
      ...
      if conn is not None:
          conn.close()
    self.logger.info("TableLogger.insertTableLog inserted row id: {}".format(row_id))
    return row_id 