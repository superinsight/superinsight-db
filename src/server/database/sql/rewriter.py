...
class SqlRewriter:
  ...
  def alterToSemanticSearchQuery(self, database, sql, similar):
    ...
    schema, table, selects, join, wheres, where_tokens, offset, limit, order_by, scoring = parser.getSelectParams()
    primary_key_values = None
    if wheres is not None and selects is not None and wheres is not None and len(wheres) > 0:
        where_conditions = parser.whereCondition(wheres)
        primary_key = sql_helper.getPrimaryKey(table_schema=schema, table_name=table)
        if primary_key is not None:
          sql = "SELECT {} FROM {}.{} {}".format(primary_key, schema, table, where_conditions)
