...
class SqlHelper:
  ...
  def isAvailable(self):
      try:
          print(self.postgres_database,self.postgres_password,self.postgres_host,self.postgres_database,self.postgres_port)
          conn = psycopg2.connect(user=self.postgres_user, password=self.postgres_password, host=self.postgres_host, port=self.postgres_port, database=self.postgres_database)
          conn.close()
          return True
      except:
          return False
  ...
  def getDatabaseConfig(self):
    ...
    self.logger.info("Database.SQL.SqlHelper.getDatabaseConfig return:{}/{}/{}".format(self.postgres_database, host, port))
    return host, port
  ...
  def read(self, sql = None, return_dict = False):
    ...
    try:
      ...
      self.logger.error("Database.SQL.SqlHelper.read:except - Error while fetching data from PostgreSQL", error)
      ...
      if conn:
        cursor.close()
        conn.close()
        self.logger.info("Database.SQL.SqlHelper.read:finally - PostgreSQL conn is closed")
      return rows
  ...
  def getPrimaryKey(self, table_schema, table_name):
    ...
    try:
      ...
      self.logger.error("Database.SQL.SqlHelper.getPrimaryKey:except - Error while fetching data from PostgreSQL", error)
      ...
      if conn:
        cursor.close()
        conn.close()
        self.logger.info("Database.SQL.SqlHelper.getPrimaryKey:finally - PostgreSQL conn is closed")
      return primary_key