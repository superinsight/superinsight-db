...
class QueryParser:
    ...
    def orderByDirection(self):
        schema, table, selects, join, wheres, where_tokens, offset, limit, order_by, scoring = self.getSelectParams()
        if order_by is None:
            return None
    ...
    def getWhereValues(self, table_schema, table_name, table_column):
        ...
        for where_token in where_tokens:
            ...
            if schema is not None and table_schema.lower() == schema.lower() and table_name.lower() == table.lower() and table_column.lower() == column.lower():
    ...
    def getSelectParams(self):
        try:
            ...
            if where_tokens is not None and len([item for item in where_tokens if item[2].lower() == "similar"]) > 0:
                scoring = True

            return schema, table, selects, join, wheres, where_tokens, offset, limit, order_by, scoring
        except:
    ...
    def getPredictParams(self):
        try:
                ...
                if self.parsed.columns_dict.get("join") is not None:
                    ...
                    for x in range(len(join)):
                        ...
                        if schema is not None and schema.upper() == self.model_schema_name:
                            ...
                            last_schema, last_table, last_column, last_values = self.parseTableColumn(target=join[x-1])
            ...
            return input_query, model_name, input_schema, input_table, input_column, input_values, limit, order_by
        except:
            print("Unexpected error:", sys.exc_info()[0])
