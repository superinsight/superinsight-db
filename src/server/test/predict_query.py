...
class TestPredictQuery(unittest.TestCase):
    ...
    def test_predict_create_output_table(self):
      sql, table_schema, table_name = SqlGenerator().scriptCreateTextClassificationPredictOutputTable(predict_id="002", labels=["comedy","drama","others"])
      print(sql)

    def test_predict_insert_output_table(self):
      sql = SqlGenerator().scriptInsertTextPredictOutputTable(predict_id="002", text="Two imprisoned men's bond over a number of year", params=[('drama', 0.8198206424713135), ('others', 0.1657235324382782), ('fantasy', 0.014455776661634445)])
      print(sql)
