...
class TextGenerationPipeline:
    ...
    def __init__(self, logger=None, handler=None, provider=None):
        ...
        self.logger.info(
            "Init ML.Pipeline.TextGeneration for provider:{}".format(str(provider))
        )
    ...
    def run(self, prompt, max_length, min_length):
        return self.__run_superinsight(
            prompt=prompt, max_length=max_length, min_length=min_length
        )
    ...
    def __run_superinsight(self, prompt, max_length, min_length):
        ...
        self.logger.info(
            "Init ML.Pipeline.TextGeneration.__run_superinsight:model_inputs",
            model_inputs,
        )
        ...
        self.logger.info(
            "Init ML.Pipeline.TextGeneration.__run_superinsight:model_output"
        )
