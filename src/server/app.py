...
async def predict_Question_Answering(database: str, predict_request: PredictRequest):
    ...
    if predict_request.sql is not None:
        ...
        values = QueryParser(sql=sql).getWhereValues(
            table_schema=Environment.model_schema_name,
            table_name=ModelTypes.QUESTION_ANSWERING.value,
            table_column="question",
        )
        if values is not None:
            question = values
        sql = rewriter.removeWhereCondition(
            full_table_name=ModelTypes.QUESTION_ANSWERING.value, sql=sql
        )
        rows = SqlHelper(database=database).read(sql)
        inputs = [row[0] for row in rows]
    table_schema, table_name = QuestionAnswering().predict(
        inputs=inputs,
        question=question,
        database=database,
        database_user=predict_request.database_user,
    )
...
async def predict_Recommender(database: str, predict_request: PredictRequest):
    ...
    table_name = None
    if predict_request.sql is not None:
        model_id = QueryParser(sql=predict_request.sql).getWhereValues(
            table_schema=Environment.model_schema_name,
            table_name=ModelTypes.RECOMMENDER.value,
            table_column="model_id",
        )
        max_size = QueryParser(sql=predict_request.sql).getWhereValues(
            table_schema=Environment.model_schema_name,
            table_name=ModelTypes.RECOMMENDER.value,
            table_column="max_size",
        )
        if model_id is not None:
            rewriter = SqlRewriter(sql=predict_request.sql)
            sql = rewriter.removeWhereCondition(full_table_name="predictions")
            sql = rewriter.removeWhereCondition(
                full_table_name=ModelTypes.RECOMMENDER.value, sql=sql
            )
            rows = SqlHelper(database=database).read(sql)
            inputs = [row[0] for row in rows]
            table_schema, table_name = Recommender().predict(
                inputs=inputs,
                database=database,
                model_id=model_id,
                database_user=predict_request.database_user,
...
async def predictSummarization(database: str, predict_request: PredictRequest):
    ...
    if predict_request.sql is not None:
        ...
        values = QueryParser(sql=sql).getWhereValues(
            table_schema=Environment.model_schema_name,
            table_name=ModelTypes.SUMMARIZATION.value,
            table_column="min_length",
        )
        if values is not None:
            min_length = int(values)
        sql = rewriter.removeWhereCondition(
            full_table_name=ModelTypes.SUMMARIZATION.value, sql=sql
        )
        rows = SqlHelper(database=database).read(sql)
        inputs = [row[0] for row in rows]
    table_schema, table_name = Summarization().predict(
        inputs=inputs,
...
async def predict_Text_Classification(database: str, predict_request: PredictRequest):
    ...
    if predict_request.sql is not None:
        ...
        values = QueryParser(sql=sql).getWhereValues(
            table_schema=Environment.model_schema_name,
            table_name=ModelTypes.TEXT_CLASSIFICATION.value,
            table_column="labels",
        )
        if values is not None:
            labels = values.split(",")
        sql = rewriter.removeWhereCondition(
            full_table_name=ModelTypes.TEXT_CLASSIFICATION.value, sql=sql
        )
        rows = SqlHelper(database=database).read(sql)
        inputs = [row[0] for row in rows]
    table_schema, table_name = TextClassification().predict(
        inputs=inputs,
        labels=labels,
        database=database,
        database_user=predict_request.database_user,
...
async def predict_Text_Generation(database: str, predict_request: PredictRequest):
    ...
    if predict_request.sql is not None:
        ...
        values = QueryParser(sql=sql).getWhereValues(
            table_schema=Environment.model_schema_name,
            table_name=ModelTypes.TEXT_GENERATION.value,
            table_column="inputs",
        )
        if values is not None:
            inputs = values
        ...
        values = QueryParser(sql=sql).getWhereValues(
            table_schema=Environment.model_schema_name,
            table_name=ModelTypes.TEXT_GENERATION.value,
            table_column="stop_word",
        )
        if values is not None:
            stop_word = values
        sql = rewriter.removeWhereCondition(
            full_table_name=ModelTypes.TEXT_GENERATION.value, sql=sql
        )
        rows = SqlHelper(database=database).read(sql)
        inputs = [row[0] for row in rows]
    table_schema, table_name = TextGeneration().predict(
        inputs=inputs,
        stop_word=stop_word,
...
async def predict_Translation(database: str, predict_request: PredictRequest):
    ...
    if predict_request.sql is not None:
        rewriter = SqlRewriter(sql=predict_request.sql)
        sql = rewriter.removeWhereCondition(full_table_name="predictions")
        source_values = QueryParser(sql=sql).getWhereValues(
            table_schema=Environment.model_schema_name,
            table_name=ModelTypes.TRANSLATION.value,
            table_column="source_language",
        )
        target_values = QueryParser(sql=sql).getWhereValues(
            table_schema=Environment.model_schema_name,
            table_name=ModelTypes.TRANSLATION.value,
            table_column="target_language",
        )
        ...
        sql = rewriter.removeWhereCondition(
            full_table_name=ModelTypes.TRANSLATION.value, sql=sql
        )
        rows = SqlHelper(database=database).read(sql)
        inputs = [row[0] for row in rows]
    table_schema, table_name = Translation().predict(
        inputs=inputs,
...
async def predict_Search(
    ...
    if sql is not None:
        ...
        values = QueryParser(sql=sql).getWhereValues(
            table_schema=Environment.model_schema_name,
            table_name=ModelTypes.SEMANTIC_SEARCH.value,
            table_column="similar",
        )
        if values is not None:
            similar = values
        else:
            similar = "*"
        sql = rewriter.removeWhereCondition(
            full_table_name=ModelTypes.SEMANTIC_SEARCH.value, sql=sql
        )
        similar, primary_key_values = rewriter.alterToSemanticSearchQuery(
            database=database, sql=sql, similar=similar
        )
    table_schema, table_name = SemanticSearch().predict(
        similar=similar,
...
async def train_Recommender(database: str, train_request: TrainRequest):
    ...
    if train_request.sql is not None:
        model_id = TrainRecommender(database=database).initialize(
            sql=train_request.sql, model_id=train_request.model_id
        )
