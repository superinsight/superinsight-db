...
class QueryParser:
    ...
    def removeWheres(self, wheres, schema_table_name_to_remove=None):
        ...
        for where in wheres:
            ...
            if (
                not "{}.{}".format(schema, table).lower()
                == schema_table_name_to_remove.lower()
            ):
            ...
            if (
                ...
                where_tokens.append(
                    self.transformToken(
                        ...
                        self.getWhereValueTokens(token),
                    )
                )
            if (
    ...
    def getCreateWithSelectStatement(self):
        try:
            ...
            for token in self.parsed.tokens:
                if (
                    token.previous_token is not None
                    and token.previous_token.previous_token is not None
                    and token.previous_token.previous_token.previous_token is not None
                    and token.previous_token.value.upper() == "AS"
                    and token.previous_token.previous_token.previous_token.value.upper()
                    == "TABLE"
                ):
        ...
        for token in self.parsed.tokens:
            if token.value.lower() == "join":
                ...
                while token_to_append is not None:
                    if (
                        len(statement) > 0
                        and token_to_append is not None
                        and token_to_append.value.lower()
                        in ["join", "where", "order", "limit"]
                    ):
                        ...
                        if exclude_statement == False:
                            if (
                                only_include_tokens is not None
                                and len(only_include_tokens) > 0
                            ):
                                for only_include_token in only_include_tokens:
                                    if (
                                        only_include_token is not None
                                        and only_include_token.lower()
                                        in statement.lower()
                                    ):
                                        join_statements = (
                                            join_statements + "\n" + statement
                        ...
                        statement, token_to_append = self.__appendNextToken(
                            statement, token_to_append
                        )
        if statement != "":
            join_statements = join_statements + "\n" + statement
    ...
    def getPredictParams(self):
        try:
            ...
            if self.parsed.columns_dict is not None:
                if self.parsed.columns_dict.get("join") is not None:
                    join_statement = self.getJoinStatements(
                        only_include_tokens=ModelTypesValidation.getList(
                            "{}.".format(self.model_schema_name)
                        )
                    )
        ...
        try:
            ...
            for token in self.parsed.tokens:
                if (
                    token.previous_token is not None
                    and token.previous_token.previous_token is not None
                    and token.previous_token.previous_token.value == "CREATE"
                    and token.previous_token.value == "MODEL"
                ):
                ...
                if (
                    token.previous_token is not None
                    and token.previous_token.previous_token is not None
                    and token.previous_token.previous_token.value == "MODEL_NAME"
                    and token.previous_token.value == "="
                ):
            ...
            if join is not None:
                for join_table in join:
                    ...
                    if (
                        schema is not None
                        and schema.upper() == self.model_schema_name
                        and ModelTypesValidation.exists(model_type=table)
                    ):
                        return "PREDICT"
        return _type
