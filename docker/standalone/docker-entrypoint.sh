...
# this is also where the database user is created, specified by `SUPERINSIGHT_USER` env
docker_init_database_dir() {
	...
	eval 'initdb --username="$SUPERINSIGHT_USER" --pwfile=<(echo "$SUPERINSIGHT_PASSWORD") '"$POSTGRES_INITDB_ARGS"' "$@"'
...
# print large warning if SUPERINSIGHT_PASSWORD is long
# error if both SUPERINSIGHT_PASSWORD is empty and POSTGRES_HOST_AUTH_METHOD is not 'trust'
# print large warning if POSTGRES_HOST_AUTH_METHOD is set to 'trust'
# assumes database is not set up, ie: [ -z "$DATABASE_ALREADY_EXISTS" ]
docker_verify_minimum_env() {
	# check password first so we can output the warning before postgres
	# messes it up
	if [ "${#SUPERINSIGHT_PASSWORD}" -ge 100 ]; then
		cat >&2 <<-'EOWARN'

			WARNING: The supplied SUPERINSIGHT_PASSWORD is 100+ characters.
	...
	if [ -z "$SUPERINSIGHT_PASSWORD" ] && [ 'trust' != "$POSTGRES_HOST_AUTH_METHOD" ]; then
		# The - option suppresses leading tabs but *not* spaces. :)
		cat >&2 <<-'EOE'
			Error: Database is uninitialized and superuser password is not specified.
			       You must specify SUPERINSIGHT_PASSWORD to a non-empty value for the
			       superuser. For example, "-e SUPERINSIGHT_PASSWORD=password" on "docker run".
		...
		cat >&2 <<-'EOWARN'
			********************************************************************************
			WARNING: POSTGRES_HOST_AUTH_METHOD has been set to "trust". This will allow
			         ...
			         a password, even if SUPERINSIGHT_PASSWORD is set. See PostgreSQL
			         ...
			         it with "-e SUPERINSIGHT_PASSWORD=password" instead to set a password in
			         "docker run".
			********************************************************************************
		EOWARN
	fi
...
docker_process_sql() {
	local query_runner=( psql -v ON_ERROR_STOP=1 --username "$SUPERINSIGHT_USER" --no-password --no-psqlrc )
...
docker_setup_env() {
	file_env 'SUPERINSIGHT_PASSWORD'

	file_env 'SUPERINSIGHT_USER' 'postgres'
	file_env 'POSTGRES_DB' "$SUPERINSIGHT_USER"
...
docker_temp_server_start() {
	...
	PGUSER="${PGUSER:-$SUPERINSIGHT_USER}" \
	pg_ctl -D "$PGDATA" \
		-o "$(printf '%q ' "$@")" \
		-w start
}
...
_main() {
	echo '################################################################################################################################################'
	echo 'Starting Superinsight Database Layer'
	################################################################################################################################################
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			export PGPASSWORD="${PGPASSWORD:-$SUPERINSIGHT_PASSWORD}"
			docker_temp_server_start "$@"
			...
			echo
			echo 'Database init process complete; ready for start up.'
			echo
		else
			echo
			echo 'Database directory appears to contain a database; Skipping initialization'
			echo
		fi
	fi
	################################################################################################################################################
	...
	nohup redis-server "$REDISDATA" >> "$LOGDATA"/redis-server.out 2>&1 &
